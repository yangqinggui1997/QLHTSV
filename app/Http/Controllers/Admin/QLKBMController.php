...
class QLKBMController extends Controller
{
    public function getDanhSach()
    {
    	try 
    	{
            ...
            $_ctdt = array();
            $cth = array();
            $_dsKhoa = khoa::select('idKhoa', 'tenKhoa', 'created_at')->orderBy('created_at', 'DESC')->get();
            foreach($_dsKhoa as $_khoa)
            {
                $khoa =  new \stdClass();
                $khoa->idKhoa = $_khoa->idKhoa;
                $khoa->tenKhoa = $_khoa->tenKhoa;
                $cb = DB::select('CALL qlk_bm__get_truong_khoa(?)', [$_khoa->idKhoa]);
                $tk = NULL;
                $cth = array();
                if(count($cb))
                {
                	$tk = new \stdClass();
	                $tk->ten = $cb[0]->hoTen;
	                $tk->anh = ($cb[0]->anh ? 'resources/images/avatars/anhcanbo/'.$cb[0]->anh : 'resources/images/avatars/user.png');
                    $tk->classTT = ((can_bo_giang_vien::where('idCB', $cb[0]->idCB)->first()->canBoGiangVienNguoiDung->nguoiDung->trangThai === 'dang_nhap') ? ' online' : '');
                }
                $khoa->truongKhoa = $tk;
                $khoa->soLuongCB = DB::select('SELECT qlk_bm__get_sl_cb_k(?) AS SLCB', [$_khoa->idKhoa])[0]->SLCB;
                $dsctdt = DB::select('CALL qlk_bm__get_ctdt_k(?)', [$_khoa->idKhoa]);
                ...
                $khoa->ctdt = (count($cth) ? $cth : NULL);
                $dsKhoa[] = $khoa;
            }
            return view('admin.hanh_chinh.ql_khoa__bo_mon', ['dsKhoa' => $dsKhoa]);
    	} 
    	catch (\Exception $ex)
    	{
    		$err = $ex->getMessage();
    ...
    public function postCapNhat(Request $request)
    {
        try
        {
            ...
            return response()->json(array('flag' => TRUE, 'maK_BM' => $maK_BM, 'per' => TRUE, 'src' => $src));
        }
        catch (\Exception $ex)
        {
            $message = $ex->getMessage();
