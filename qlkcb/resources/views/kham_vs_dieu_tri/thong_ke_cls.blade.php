...
@section('content')
    <div class="main-content">
        ...
        <input type="hidden" id="phong" value="{{$nd->nhanVien->phongBan->TenPhong}}" data-sp="{{$nd->nhanVien->phongBan->SoPhong}}">
                                    ...
                                    <div class="table-data__tool-left">
                                        ...
                                        <div class="rs-select2--light width-200px m-b-15">
                                            <select class="js-select2" id="khoa_f">
                                                ...
                                                <option value="<?php echo $k->IdKhoa; ?>">{{$k->TenKhoa}}</option>
                                                    <?php }?>
                                                @endif
                                            </select>
                                            <div class="dropDownSelect2"></div>
                                        ...
                                        <div class="rs-select2--light width-230px m-b-15">
                                            <div class="form-group">
                                                <input type="text" list="dscls" id="tencls" placeholder="Nhập tên dịch vụ CLS" class="form-control" data-toggle="tooltip" title="Nhập chính xác tên dịch vụ CLS!"/>
                                                <datalist id="dscls">
                                                    @if(isset($dscls))
                                                    @foreach($dscls as $cls)
                                                    <option value="{{$cls->IdDMCLS}}" data-value="{{$cls->IdDMCLS}}">{{$cls->TenCLS}}</option>
                                                    @endforeach
                                                    @endif
                                                </datalist>
                                    ...
                                    <div class="card" style="font-family: 'Noto Serif'; font-size: 10pt; font-weight: normal;">
                                        <div class="card-body card-block" id="tqarea">
                                            <div class="row">
                                                ...
                                                <div class="col-lg-3">
                                                    <div class="form-group">
                                                        <label class="form-control-label" style="font-weight: 600">BHYT</label>
                                                        <input type="text" readonly="" class="form-control" id="khambhyt">
                                                    </div>
                                                </div>
                                                <div class="col-lg-2">
                                                    <div class="form-group">
                                                        <label class="form-control-label" style="font-weight: 600">Thu phí</label>
...
<script type="text/javascript">
    $(function () {
        ...
        var tgt='', tgty='', quy='', tgbd='', tgkt='', khoa='', huyen='', xa='', title='', tgtaotk='', flagcls=false;
        ...
        $('#btnlocds').click(function (){
            ...
            var cls=$('#tencls_hide').val(), tencls=$('#tencls').val();
            if(tencls.toString().trim() == ''){
                alert('Vui lòng nhập tên dịch vụ CLS trước khi thống kê!');
                return false;
            }
            if(flagcls == false){
                alert('Dịch vụ CLS này hiện không được áp dụng tại bệnh viện, nếu là dịch vụ mới hãy thêm một số thông tin của loại dịch vụ này trước khi thống kê!');
                return false;
            }
            var formData = new FormData();
            formData.append('_token', CSRF_TOKEN);
            if($('#filtertime').val() == 0)
            {
                ...
                file_name_print='TK_TUY_Y_TG_PHONG_'+$('#phong').val().toString().toUpperCase()+'('+$('#phong').attr('data-sp').toString().toUpperCase()+')_DVCLS_'+tencls.toString().toUpperCase()+'_'+d.getDate()+'_'+thang+'_'+d.getFullYear()+'_'+gio+'_'+phut+'_'+giay+'_'+apm;
            }
            else if($('#filtertime').val() == 1)
            {
                tgt='ngay'; tgty=$('#thoigankty').val();
                formData.append('tgt', 'ngay');
                file_name_print='TK_NGAY_PHONG_'+$('#phong').val().toString().toUpperCase()+'('+$('#phong').attr('data-sp').toString().toUpperCase()+')_DVCLS_'+tencls.toString().toUpperCase()+'_'+d.getDate()+'_'+thang+'_'+d.getFullYear()+'_'+gio+'_'+phut+'_'+giay+'_'+apm;
            }
            else if($('#filtertime').val() == 2)
            {
                tgt='thang'; tgty=$('#thoigankty').val();
                formData.append('tgt', 'thang');
                file_name_print='TK_THANG_PHONG_'+$('#phong').val().toString().toUpperCase()+'('+$('#phong').attr('data-sp').toString().toUpperCase()+')_DVCLS_'+tencls.toString().toUpperCase()+'_'+d.getDate()+'_'+thang+'_'+d.getFullYear()+'_'+gio+'_'+phut+'_'+giay+'_'+apm;
            }
            else if($('#filtertime').val() == 3)
            {
                ...
                file_name_print='TK_QUY_'+q+'_PHONG_'+$('#phong').val().toString().toUpperCase()+'('+$('#phong').attr('data-sp').toString().toUpperCase()+')_DVCLS_'+tencls.toString().toUpperCase()+'_'+d.getDate()+'_'+thang+'_'+d.getFullYear()+'_'+gio+'_'+phut+'_'+giay+'_'+apm;
            }
            else
            {
                tgt='nam'; tgty=$('#thoigankty').val();
                formData.append('tgt', 'nam');
                file_name_print='TK_NAM_PHONG_'+$('#phong').val().toString().toUpperCase()+'('+$('#phong').attr('data-sp').toString().toUpperCase()+')_DVCLS_'+tencls.toString().toUpperCase()+'_'+d.getDate()+'_'+thang+'_'+d.getFullYear()+'_'+gio+'_'+phut+'_'+giay+'_'+apm;
            }
            ...
            $.ajax({
                type: 'POST',
                dataType: "JSON",
                url: '/qlkcb/kham_va_dieu_tri/thong_ke_cls/them_tk_cls',
                xhr: function() {
                    var myXhr = $.ajaxSettings.xhr();
                    return myXhr;
                },
                ...
                success: function(data) {
                    ...
                    title='BẢNG THỐNG KÊ THỰC HIỆN DV CẬN LÂM SÀNG ['+tencls.toString().toUpperCase()+'] - PHÒNG '+$('#phong').val().toString().toUpperCase()+' ('+$('#phong').attr('data-sp').toString().toUpperCase()+')';
                                    ...
                                    <table class="table table-bordered" style="font-size: 10pt;">\n\
                                        <thead style="font-size: 8pt; vertical-align: middle">\n\
                                            <tr>\n\
                                                ...
                                                <th><center>KHOA</center></th>\n\
                                                <th><center>TỔNG LƯỢT THỰC HIÊN CLS</center></th>\n\
        ...
        $('#btnlamlai').click(function(){
            $('#tongso').val('');$('#quyarea').addClass('hidden');$('#quy').val('');$('#tencls').val('');
            $('#tqarea').slideUp(800);       
...
//        end
        ...
        $('input[list="dscls"]').on('input', function(e) {
            var input = e.target,
