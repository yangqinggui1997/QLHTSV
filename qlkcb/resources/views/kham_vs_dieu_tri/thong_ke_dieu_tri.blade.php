...
@section('content')
    <div class="main-content">
        ...
        <input type="hidden" id="khoa" value="{{$nd->nhanVien->phongBan->Khoa->TenKhoa}}">
                    ...
                    <div class="container">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="m-b-35">
                                    <?php  ?> 
                                    <h3 class="title-5 font-weight-bold text-green">THỐNG KÊ - KHOA [{{mb_convert_case($nd->nhanVien->phongBan->Khoa->TenKhoa, MB_CASE_UPPER, 'UTF-8')}}] - BS. {{mb_convert_case($nd->nhanVien->TenNV, MB_CASE_UPPER, 'UTF-8')}}</h3>
                                    ...
                                    <div class="table-data__tool-left">
                                        ...
                                        <div class="rs-select2--light width-230px m-b-15">
                                            <div class="form-group">
                                                <input type="text" list="dstenbenh" id="tenbenh" placeholder="Nhập tên bệnh" class="form-control" data-toggle="tooltip" title="Nhập chính xác tên bệnh!"/>
                                                <datalist id="dstenbenh">
                                                    @if(isset($dsbenh))
                                                    @foreach($dsbenh as $benh)
                                                    <option value="{{$benh->IdBenh}}" data-value="{{$benh->IdBenh}}">{{$benh->TenBenh}}</option>
                                                    @endforeach
                                                    @endif
                                                </datalist>
                                                <input type="hidden" id="tenbenh_hide">
                                    ...
                                    <div class="card" style="font-family: 'Noto Serif'; font-size: 10pt; font-weight: normal;">
                                        <div class="card-body card-block" id="tqarea">
                                            <div class="row">
                                                ...
                                                <div class="col-lg-2">
                                                    <div class="form-group">
                                                        <label class="form-control-label" style="font-weight: 600">ĐT nội trú</label>
                                                        <input type="text" readonly="" class="form-control" id="noitru">
                                                    </div>
                                                </div>
                                                <div class="col-lg-2">
                                                    <div class="form-group">
                                                        <label class="form-control-label" style="font-weight: 600">ĐT ngoại trú</label>
    ...
    $(function () {
        ...
        $('#tinh_f').change(function(){
            ...
            $.ajax({
                ...
                error: function (jqXHR, textStatus, errorThrown) {
                    if(jqXHR.status == 419){
                        alert("Không thể lấy danh sách quận - huyện! Người dùng không được xác thực (có thể đã đăng xuất).");
                    }
                    else if(jqXHR.status == 500){
                        alert("Không thể lấy danh sách quận - huyện! Đã phát hiện lỗi trên máy chủ phục vụ.");
                    }
                    else{
                        alert("Không thể lấy danh sách quận - huyện! Lỗi: "+jqXHR.responseText+" | "+textStatus+" | "+errorThrown);
                    }
                }
            });
        });
        ...
        $('#huyen_f').change(function(){
            ...
            $.ajax({
                ...
                error: function (jqXHR, textStatus, errorThrown){
                    if(jqXHR.status == 419){
                        alert("Không thể lấy danh sách xã - phường! Người dùng không được xác thực (có thể đã đăng xuất).");
                    }
                    else if(jqXHR.status == 500){
                        alert("Không thể lấy danh sách xã - phường! Đã phát hiện lỗi trên máy chủ phục vụ.");
                    }
                    else{
                        alert("Không thể lấy danh sách xã - phường! Lỗi: "+jqXHR.responseText+" | "+textStatus+" | "+errorThrown);
                    }
                }
            });
        });
        ...
        $('#btnlocds').click(function (){
            ...
            var benh=$('#tenbenh_hide').val(), tenbenh=$('#tenbenh').val();
            ...
            if($('#filtertime').val() == 0)
            {
                ...
                file_name_print='TK_TUY_Y_TG_KHOA_'+$('#khoa').val().toString().toUpperCase()+'_BENH_'+tenbenh.toString().toUpperCase()+'_'+d.getDate()+'_'+thang+'_'+d.getFullYear()+'_'+gio+'_'+phut+'_'+giay+'_'+apm;
            }
            else if($('#filtertime').val() == 1)
            {
                tgt='ngay'; tgty=$('#thoigankty').val();
                formData.append('tgt', 'ngay');
                file_name_print='TK_NGAY_KHOA_'+$('#khoa').val().toString().toUpperCase()+'_BENH_'+tenbenh.toString().toUpperCase()+'_'+d.getDate()+'_'+thang+'_'+d.getFullYear()+'_'+gio+'_'+phut+'_'+giay+'_'+apm;
            }
            else if($('#filtertime').val() == 2)
            {
                tgt='thang'; tgty=$('#thoigankty').val();
                formData.append('tgt', 'thang');
                file_name_print='TK_THANG_KHOA_'+$('#khoa').val().toString().toUpperCase()+'_BENH_'+tenbenh.toString().toUpperCase()+'_'+d.getDate()+'_'+thang+'_'+d.getFullYear()+'_'+gio+'_'+phut+'_'+giay+'_'+apm;
            }
            else if($('#filtertime').val() == 3)
            {
                ...
                file_name_print='TK_QUY_'+q+'_KHOA_'+$('#khoa').val().toString().toUpperCase()+'_BENH_'+tenbenh.toString().toUpperCase()+'_'+d.getDate()+'_'+thang+'_'+d.getFullYear()+'_'+gio+'_'+phut+'_'+giay+'_'+apm;
            }
            else
            {
                tgt='nam'; tgty=$('#thoigankty').val();
                formData.append('tgt', 'nam');
                file_name_print='TK_NAM_KHOA_'+$('#khoa').val().toString().toUpperCase()+'_BENH_'+tenbenh.toString().toUpperCase()+'_'+d.getDate()+'_'+thang+'_'+d.getFullYear()+'_'+gio+'_'+phut+'_'+giay+'_'+apm;
            }
            ...
            if(tenbenh.toString().trim() != ''){
                if(flagbenh == false){
                    alert('Bệnh này hiện không được quản lý trong khoa, nếu là bệnh mới hãy thêm một số thông tin của loại bệnh này trước khi thống kê!');
                    return false;
                }
                formData.append('benh', benh);
                formData.append('pl', 'tkb');
            }
            else{
                formData.append('pl', 'tkdp');
                if(tinh == "all"){
                    alert('Mặc định khi không cung cấp tên loại bệnh hệ thống sẽ thống kê các loại bệnh theo địa phương, vì vậy bạn nên chọn một địa phương để thống kê!');
                    return false;
                }
            ...
            var tkdp='tinh';
            if(tinh != 'all' && huyen == 'all'){
                tkdp='huyen';
            }
            else if(huyen != 'all' || xa != 'all'){
                tkdp='xa';
            }
            $.ajax({
                type: 'POST',
                dataType: "JSON",
                url: '/qlkcb/kham_va_dieu_tri/thong_ke_dieu_tri/them_tk_dt',
                xhr: function() {
                    var myXhr = $.ajaxSettings.xhr();
                    return myXhr;
                },
                ...
                data: formData,
                success: function(data) {
                    // alert(data.msg+"|"+data.arr);
                    // $('#tbl_tk').html(data.msg);
                    if(data.msg != 'tc')
                    {
                    ...
                    $('#tongso').val(data.slt);$('#khambhyt').val(data.slbh);$('#khamthuphi').val(data.sltp);
                    $('#noitru').val(data.slnoi);$('#ngoaitru').val(data.slngoai);
                    ...
                    if(tenbenh.toString().trim() != ''){
                        $('#loaitk').text('địa phương');
                        title='BẢNG THỐNG KÊ BỆNH ['+tenbenh.toString().toUpperCase()+'] - KHOA '+$('#khoa').val().toString().toUpperCase();
                    }
                    else{
                        $('#loaitk').text('loại bệnh');
                        if(tinh!='all' && huyen=='all' && xa == 'all'){
                            title='BẢNG THỐNG KÊ TÌNH HÌNH BỆNH TẬT TẠI TỈNH ['+$('#tinh_f option[value="'+tinh+'"]').text().toString().toUpperCase()+'] - KHOA '+$('#khoa').val().toString().toUpperCase();
                        }
                        else if(huyen!='all' && xa == 'all'){
                            title='BẢNG THỐNG KÊ TÌNH HÌNH BỆNH TẬT TẠI HUYỆN '+'['+$('#huyen_f option[value="'+huyen+'"]').text().toString().toUpperCase()+']'+' TỈNH ['+$('#tinh_f option[value="'+tinh+'"]').text().toString().toUpperCase()+'] - KHOA '+$('#khoa').val().toString().toUpperCase();
                        }
                        else{
                            title='BẢNG THỐNG KÊ TÌNH HÌNH BỆNH TẬT TẠI XÃ ['+$('#xa_f option[value="'+xa+'"]').text().toString().toUpperCase()+'] HUYỆN ['+$('#huyen_f option[value="'+huyen+'"]').text().toString().toUpperCase()+'] TỈNH ['+$('#tinh_f option[value="'+tinh+'"]').text().toString().toUpperCase()+'] - KHOA '+$('#khoa').val().toString().toUpperCase();
                        }
                    }
                    ...
                    <div class="card-body card-block printcontent" style="min-height: 1300px; max-height: 1300px;">\n\
                        ...
                        <div class="row">\n\
                            ...
                            <div class="col-lg-2">\n\
                                <label class="form-control-label">Đối tượng thu phí:</label>\n\
                            </div>\n\
                            <div class="col-lg-2">\n\
                                <label style="font-weight: 600">'+data.sltp+'</label>\n\
                            </div>\n\
                            <div class="col-lg-2">\n\
                                <label class="form-control-label">Điều trị nội trú:</label>\n\
                            </div>\n\
                            <div class="col-lg-2">\n\
                                <label style="font-weight: 600">'+data.slnoi+'</label>\n\
                            </div>\n\
                            <div class="col-lg-2">\n\
                                <label class="form-control-label">Điều trị ngoại trú:</label>\n\
                            </div>\n\
                            <div class="col-lg-2">\n\
                                <label style="font-weight: 600">'+data.slngoai+'</label>\n\
                            </div>\n\
                        </div>\n\
                        <hr class="line-seprate">\n\
                        <div class="row" style="margin-top: 15px;">\n\
                                <div class="col-lg-12">\n\
                                    ...
                                    if(tenbenh.toString().trim() != ''){
                                        if(tkdp == 'tinh'){
                                            content+='<th><center>ĐỊA PHƯƠNG (TỈNH)</center></th>';
                                        }
                                        else if(tkdp == 'huyen'){
                                            content+='<th><center>ĐỊA PHƯƠNG (HUYỆN)</center></th>';
                                        }
                                        else{
                                            content+='<th><center>ĐỊA PHƯƠNG (XÃ)</center></th>';
                                        }
                                    }
                                    else{
                                        content+='<th><center>LOẠI BỆNH</center></th>';
                                    }
                                        
                                                content+='\n\
                                                <th><center>TỔNG SỐ BỆNH NHÂN</center></th>\n\
                                                <th><center>BHYT</center></th>\n\
                                                <th><center>THU PHÍ</center></th>\n\
                                                <th><center>NỘI TRÚ</center></th>\n\
                                                <th><center>NGOẠI TRÚ</center></th>\n\
                                            </tr>\n\
                                        </thead>\n\
                    ...
                    <div class="card-body card-block printcontent" style="min-height: 1300px; max-height: 1300px;">\n\
                        <div class="row" style="margin-top: 15px;">\n\
                                <div class="col-lg-12">\n\
                                    ...
                                    if(tenbenh.toString().trim() != ''){
                                        if(tkdp == 'tinh'){
                                            newpage_bf+='<th><center>ĐỊA PHƯƠNG (TỈNH)</center></th>';
                                        }
                                        else if(tkdp == 'huyen'){
                                            newpage_bf+='<th><center>ĐỊA PHƯƠNG (HUYỆN)</center></th>';
                                        }
                                        else{
                                            newpage_bf+='<th><center>ĐỊA PHƯƠNG (XÃ)</center></th>';
                                        }
                                    }
                                    else{
                                        newpage_bf+='<th><center>LOẠI BỆNH</center></th>';
                                    }
                                                newpage_bf+='\n\
                                                <th><center>TỔNG SỐ BỆNH NHÂN</center></th>\n\
                                                <th><center>BHYT</center></th>\n\
                                    ...
                                    content+='\n\
                                        ...
                                        <div class="row m-t-45 text-center">\n\
                                            <div class="col-lg-4">\n\
                                                ...
                                                <label style="font-size: 9pt; font-weight: 600;">BS. '+data.tennv.toString().toUpperCase()+'</label>\n\
                                            </div>\n\
                                            <div class="col-lg-4">\n\
                                                <label style="margin-bottom: 0;">An Giang, ngày......., tháng......., năm.......</label><br>\n\
        ...
        $('#btnlamlai').click(function(){
            $('#tongso').val('');$('#quyarea').addClass('hidden');$('#quy').val('');$('#tenbenh').val('');
            $('#tqarea').slideUp(800);       
            $('#filtertime').val(0);$('#filtertime').change();$('#tinh_f').val('all');$('#tinh_f').change();
...
//        end
        ...
        //xử lý lấy phần text cho datalist
        $('input[list="dstenbenh"]').on('input', function(e) {
            var input = e.target,
                list = input.getAttribute('list'),
                options = document.querySelectorAll('#' + list + ' option'),
                hiddenInput = document.getElementById('tenbenh_hide'),
                inputValue = input.value;

            for(var i = 0; i < options.length; i++) {
                var option = options[i];
                if(option.innerText === inputValue || option.getAttribute('data-value') === inputValue) {
                    flagbenh=true;
