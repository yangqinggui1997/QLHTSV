        ...
        <section class="p-t-20">
            <div class="container">
                <div class="row">
                    <div class="col-md-12">
                        <div class=" m-b-35">
                            <h3 class="title-5 font-weight-bold text-green">DANH SÁCH CHỈ ĐỊNH PHẪU THUẬT - KHOA [{{mb_convert_case($nd->nhanVien->phongBan->Khoa->TenKhoa, MB_CASE_UPPER, 'UTF-8')}}] - BS. {{mb_convert_case($nd->nhanVien->TenNV, MB_CASE_UPPER, 'UTF-8')}}</h3>
                            ...
                            <table class="table table-data2 table-hover m-b-20 text-center">
                                <thead>
                                    <tr>
                                        ...
                                        <th>phòng thực hiện phẫu thuật</th>
                                        <th>nhân viên phẫu thuật chính</th>
                                        <th>Thao tác</th>
                                    </tr>
                                </thead>
                                <tbody id="tbl_pt">
                                    ...
                                    @foreach($dspt as $pt)
                                        <tr class="tr-shadow">
                                            ...
                                            <td class="text-left">
                                                @foreach($pt->benhAnNoiTruCT->benhAnNoiTru->chuanDoan as $cd)
                                                    {{'- '.$cd->danhMucBenh->TenBenh}}<br>
                                                @endforeach
                                            </td>
                                            <td>
                                                {{$pt->danhMucCLS->TenCLS}}
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                <?php
                                                    echo 'Phòng số '.$pt->phongBan->SoPhong.' - '.$pt->phongBan->TenPhong;
                                                ?>
                                            </td>
                                            <td>
                                                {{$pt->nhanVien->TenNV}}
                                            ...
                                            <td>
                                                <div class="table-data-feature">
                                                    <button type="button" class="item" data-toggle="tooltip" data-placement="top" title="Xóa" data-button="btnxoa" data-id="{{$pt->IdPT}}" data-name="{{$pt->benhAnNoiTruCT->benhAnNoiTru->phieuDKKham->phieuDKKham->benhNhan->HoTen}}">
...
<script>
    $(function () {
        ...
        $('#tbl_pt').on('click','button[data-button="btnxoa"]',function(){
            var id=$(this).attr('data-id');
            var name=$(this).attr('data-name');
            var cf=confirm("Bạn có thực sự muốn xóa chỉ định phẫu thuật của bệnh nhân "+name+"?");
            if(cf==true){
                var formData = new FormData();
                formData.append('_token', CSRF_TOKEN);
                formData.append('idpt', id);
                        ...
                        if(data.msg == 'tc'){
                            ...
                            if(locds == true){
                                ...
                                if(soluongl == 0){
                                    ...
                                    $('#kqtimliem').text("Có "+soluongl+" phiếu chỉ định phẫu thuật được tìm thấy!");
                                }
                            }
                            else{
                                if(tk == true){
                                    ...
                                    if(soluongtk == 0){
                                        ...
                                        $('#kqtimliem').text("Có "+soluongtk+" phiếu chỉ định phẫu thuật tìm thấy!");
                                    }
                                }
                            }
                            $('#tbl_pt tr').has('td div button[data-id="'+id+'"]').next('tr.spacer').remove();
                            $('#tbl_pt tr').has('td div button[data-id="'+id+'"]').remove();
                            alert("Xóa phiếu chỉ định phẫu thuật thành công!");
                        }
                        else{
                            alert("Xóa phiếu chỉ định phẫu thuật thất bại! Lỗi: "+data.msg);
                        }
                    },
                    error: function(jqXHR, textStatus, errorThrown) {
                        if(jqXHR.status == 419){
                            alert("Xóa phiếu chỉ định phẫu thuật thất bại! Người dùng không được xác thực (có thể đã đăng xuất).");
                        }
                        else if(jqXHR.status == 500){
                            alert("Xóa phiếu chỉ định phẫu thuật thất bại! Đã phát hiện lỗi trên máy chủ phục vụ.");
                        }
                        else{
                            alert("Xóa phiếu chỉ định phẫu thuật thất bại! Lỗi: "+jqXHR.responseText+" | "+textStatus+" | "+errorThrown);
                        }
                    }
                });
            }
        ...
        $('#btnxoatc').click(function(){
            if(!$('#tbl_pt input[data-input="check"]').is(":checked")){
                alert("Bạn chưa chọn phiếu chỉ định phẫu thuật để xóa!");
                return false;
            }
            else{
               var name='';var arr=[],arr_name=[]
                $('#tbl_pt input[data-input="check"]').each(function(){
                    if($(this).is(":checked")){
                ...
                var cf;
                if(arr_name.length > 1){
                    cf=confirm("Bạn có thực sự muốn xóa các phiếu chỉ định phẫu thuật của các bệnh nhân "+name+"?");
                }
                else
                {
                    cf=confirm("Bạn có thực sự muốn xóa phiếu chỉ định phẫu thuật của bệnh nhân "+name+"?");
                }
                if(cf==true){
                    var formData = new FormData();
                    formData.append('_token', CSRF_TOKEN);
                    formData.append('idpt', arr);
                    $.ajax({
                        type: 'POST',
                        dataType: "JSON",
                        url: '/qlkcb/kham_va_dieu_tri/chi_dinh_phau_thuat/xoa',
                        ...
                        success: function(data) {
                            ...
                            if(data.msg == 'tc'){
                                if(arr.length > 1){
                                    if(locds == true){
                                        ...
                                        if(tk == true){
                                            ...
                                            {
                                                $('#kqtimliem').text("Có "+soluongtk+" phiếu chỉ định phẫu thuật được tìm thấy!");
                                            }
                                        }
                                    }
                                    for (var i = 0; i < arr.length; i++) {
                                        $('#tbl_pt tr').has('td div button[data-id="'+arr[i]+'"]').next('tr.spacer').remove();
                                        $('#tbl_pt tr').has('td div button[data-id="'+arr[i]+'"]').remove();
                                    }
                                    if($('#tbl_pt').children().length == 0){
                                        $('input[data-input="checksum"]').prop("checked",false);
                                    }
                                    alert("Xóa thông tin các phiếu chỉ định phẫu thuật thành công!");
                                    
                                }
                                else
                                {
                                    if(locds == true){
                                        $('#kqtimliem').text("Có "+(soluongl - 1)+" phiếu chỉ định phẫu thuật được tìm thấy!");
                                    }
                                    else{
                                        if(tk == true){
                                            $('#kqtimliem').text("Có "+(soluongtk - 1)+" phiếu chỉ định phẫu thuật được tìm thấy!");
                                        }
                                    }
                                    $('#tbl_pt tr').has('td div button[data-id="'+arr[0]+'"]').next('tr.spacer').remove();
                                    $('#tbl_pt tr').has('td div button[data-id="'+arr[0]+'"]').remove();
                                    
                                    if($('#tbl_pt').children().length == 0){
                                        $('input[data-input="checksum"]').prop("checked",false);
                                    }
                                    alert("Xóa thông tin phiếu chỉ định phẫu thuật thành công!");
                                ...
                                if(arr.length > 1)
                                {
                                    alert("Xóa thông tin các phiếu chỉ định phẫu thuật thất bại! Lỗi: "+data.msg);
                                }
                                else
                                {
                                    alert("Xóa thông tin phiếu chỉ định phẫu thất bại! Lỗi: "+data.msg);
                                }
                            }
                        },
                        error: function(jqXHR, textStatus, errorThrown) {
                            ...
                            {
                                if(jqXHR.status == 419){
                                    alert("Xóa thông tin các phiếu chỉ định phẫu thuật thất bại! Người dùng không được xác thực (có thể đã đăng xuất).");
                                }
                                else if(jqXHR.status == 500){
                                    alert("Xóa thông tin các phiếu chỉ định phẫu thuật thất bại! Đã phát hiện lỗi trên máy chủ phục vụ.");
                                }
                                else{
                                    alert("Xóa thông tin các phiếu chỉ định phẫu thuật thất bại! Lỗi: "+jqXHR.responseText+" | "+textStatus+" | "+errorThrown);
                                }
                            }
                            else
                            {
                                if(jqXHR.status == 419){
                                    alert("Xóa thông tin phiếu chỉ định phẫu thuật thất bại! Người dùng không được xác thực (có thể đã đăng xuất).");
                                }
                                else if(jqXHR.status == 500){
                                    alert("Xóa thông tin phiếu chỉ định phẫu thuật thất bại! Đã phát hiện lỗi trên máy chủ phục vụ.");
                                }
                                else{
                                    alert("Xóa thông tin phiếu chỉ định phẫu thuật thất bại! Lỗi: "+jqXHR.responseText+" | "+textStatus+" | "+errorThrown);
                                }
                            }
                        }
                    });
        ...
        $('#btntimkiem').click(function (){
            ...
            $.ajax({
                type: 'POST',
                dataType: "JSON",
                url: '/qlkcb/kham_va_dieu_tri/chi_dinh_phau_thuat/tim_kiem',
                xhr: function() {
                    var myXhr = $.ajaxSettings.xhr();
                    return myXhr;
                },
                ...
                success: function(data) {
                    ...
                    if(data.msg != 'tc'){
                        ...
                        if(data.sl > 0){
                            ...
                            $('#tbl_pt').html(tt_cd);
                            $('#tbl_pt button[data-button]').tooltip({
                                trigger: 'manual'
                            ...
                            $('#kqtimliem').text("Có "+data.sl+" phiếu chỉ định phẫu thuật được tìm thấy!");
//                            $('#btnlocds').tooltip('hide').attr('data-original-title', 'Lọc danh sách tìm kiếm').tooltip('fixTitle').tooltip('show');
                        }
                        else{
                            $('#tbl_pt').html("");
                            $('#kqtimliem').text("Không có phiếu chỉ định phẫu thuật nào được tìm thấy!");tk=false;
                        }
                    }
        ...
        $('#btnnapds').click(function(){
            ...
            $.ajax({
                type: 'POST',
                dataType: "JSON",
                url: '/qlkcb/kham_va_dieu_tri/chi_dinh_phau_thuat/lay_ds_all',
                xhr: function() {
                    var myXhr = $.ajaxSettings.xhr();
                    return myXhr;
                },
                ...
                success: function(data) {
                    // Success
                    if(data.msg != 'tc'){
                        alert("Lỗi khi tải danh sách phiếu chỉ định phẫu thuật! Mô tả: "+data.msg);
                    }else{
                        var tt_cd='';
                        for(var i=0; i<data.dscd.length; ++i){
                            tt_cd+='\n\
...
//                        $('#btnlocds').tooltip('hide').attr('data-original-title', 'Lọc danh sách').tooltip('fixTitle').tooltip('show');
        ...
        $('body').on('change', 'input[data-input="checksum"]', function(){
            if($(this).prop("checked")){
                $('#tbl_pt input[data-input="check"]').prop("checked",true);
            }
            else{
                $('#tbl_pt input[data-input="check"]').prop("checked",false);
            }
        });
        //end
        
        //click check
        $('#tbl_pt').on('change', 'input[data-input="check"]', function(){
            if(!$(this).prop("checked")){
                $('input[data-input="checksum"]').prop("checked",false);
            }
            else{
                if($('#tbl_pt input[data-input="check"]:checked').length == $('#tbl_pt input[data-input="check"]').length){
