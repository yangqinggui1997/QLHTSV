...
$(function(){
	try
	{
		...
		function init_DataTablesKQRL(){
			try
			{
				if(console.log("run_datatablesKQRL"),"undefined"!=typeof $.fn.DataTable)
				{
	    			var b = $('#table__id__ketQuaHTRL');
	    			var count = 0;
	    			/*tạo table = script sau đó khởi tạo, insert table vào vùng chứa*/
	    			console.log("init_DataTablesKQRL");
    				b.dataTable({
	    				order:[[1,"asc"]],
	    				dom:"Blfrtip",
	    				buttons:[
		...
		if($('#tbody__id__sinhVien').length)
			$('#tbody__id__sinhVien').on('click', '[data-button="xemKQRLSV"]', function(){
				try
	            {
